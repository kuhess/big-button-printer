...
bool Tprinter::getPixel(const uint8_t *bitmap, uint16_t width, uint16_t height, uint16_t x, uint16_t y) {
    if (x >= width || y >= height) {
        return false;
    }

    uint32_t global_bit_idx = (y * width) + x;

    uint32_t byte_index = global_bit_idx / 8;
    ushort local_bit_index = global_bit_idx % 8;


    return ((bitmap[byte_index]) >> (7 - local_bit_index)) & 0x01;
}

void Tprinter::printBitmap2(const uint8_t *bitmap, uint16_t width, uint16_t height) {
    uint8_t mode = 33;
    uint8_t band_height = 24;

    uint8_t nL = width % 256;
    uint8_t nH = width / 256;
    ...
    while (row < height) {
        ...
        for (uint16_t x = 0; x < width; x++) {
            // OPTIM: here we could transpose 8x8 bits instead of reading each bit individually
            for (uint8_t k = 0; k < 3; k++) {
                ...
                for (uint8_t b = 0; b < 8; b++) {
                    ...
                    bool pixel_value = getPixel(bitmap, width, height, x, y);

                    slice |= (byte) ((pixel_value ? 1 : 0) << (7 - b));
                }
